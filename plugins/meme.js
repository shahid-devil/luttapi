    ...
    MyPnky.addCommand({pattern: 'meme ?(.*)', fromMe: false, desc: Lang.MEMES_DESC}, (async (message, match) => {    
	 ...
	 if (match[1].includes('{1}')) {    
	    ...
	    fontSize: 150, 	
            ...
            if(err) throw new Error(err)
            await message.client.sendMessage(message.jid, fs.readFileSync('SHAZZ-LUTTAPI.png'), MessageType.image, {filename: 'LUTTAPI_meme.png', mimetype: Mimetype.png, caption: Config.AFN});
            await info.delete();    
        });
       }
